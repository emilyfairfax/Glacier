...
% Topography and Exaggerated Erosion
...
%Variable Arrays
    ...
    zrock = 3000+(topomax-3000)*exp(-.5*m*10^-3*x);
...
for i=1:1:imax
    ...
    % Calculate Erosion
    for k = 1:(length(icethickness)-1)
    if icethickness(k) > 0
        erosionrate(k) = a*Uslide(k).^n;
        zrock(k) = zrock(k) - erosionrate(k)*dt;
    end
    end
    ...
    if rem(t(i),tplot)==0  
        ...
        %Glacier Profile Subplot
            ...
            title('Glacier Profile in Relation to ELA through Time, with Exaggerated Erosion Rates')
